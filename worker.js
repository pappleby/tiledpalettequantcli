...
function quantizeImage(image) {
    inspector.console.log(quantizationOptions);
    ...
    inspector.console.log("Colors per tile: " + avgPixelsPerTile.toFixed(2));
    ...
    for (let i = 0; i < replaceIterations; i++) {
        ...
        inspector.console.log("MSE: " + mse.toFixed(0));
        // inspector.console.log((performance.now() - t1).toFixed(0) + " ms");
    }
    if (useMin) {
    ...
    inspector.console.log("Normal final: " + meanSquareError(palettes, tiles).toFixed(0));
    inspector.console.log("Dither final: " + meanSquareErrorDither(palettes, tiles).toFixed(0));
    ...
    inspector.console.log("> MSE: " + meanSquareError(palettes, tiles).toFixed(2));
    inspector.console.log(`> Time: ${((performance.now() - t0) / 1000).toFixed(2)} sec`);
    return resultImage;
}
...
function replaceWeakestColors(palettes, tiles, minColorFactor, minPaletteFactor, replacePalettes) {
        ...
        for (let palIndex = 0; palIndex < palettes.length; palIndex++) {
            ...
            for (let i = 0; i < palettes[palIndex].length; i++) {
                if (i == minColorIndex && shouldReplaceMinColor) {
                    inspector.console.log("replaced color in palette " + palIndex);
    ...
    if (replacePalettes &&
        ...
        inspector.console.log("replaced palette " + minPaletteIndex);
...
function extractTiles(image) {
    ...
    inspector.console.log("avg pixels per tile: " + avgPixelsPerTile.toFixed(2));
    return tiles;
}
...
function runQuantization(imageData, inputQuantizationOptions) {
    
    quantizationOptions = inputQuantizationOptions;
    return quantizeImage(imageData);
}

module.exports = { runQuantization };